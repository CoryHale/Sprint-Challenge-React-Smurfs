...
- [X] Explain the differences between `client-side routing` and `server-side routing`.
  ...
  The biggest difference between client-side and server-side routing is that you do not have to send in `get` requests for client-side routing, all of that happens internally in the application making for a faster, smoother experience, whereas every time you would need to route server-side another `get` request is sent, which generally takes more time.

- [X] Which HTTP methods can be mapped to the CRUD acronym that we use when interfacing with APIs/Servers.

  There are 3 HTTP methods that we can use and it is as follows:
    `CR` stands for `CREATE` and is where we use `.post()`
    `U` stands for `UPDATE` and is where we use `.put()`
    `D` stands for `DELETE` and is where we use `.delete()`

- [X] Mention three tools we can use to make AJAX requests.

  The 3 tools are `Axios`, `jQuery`, and `SuperAgent`
